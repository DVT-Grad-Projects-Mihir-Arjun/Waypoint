...
- 2026-08-24: Post-PR CI fix. The `windows-latest` leg of PR #9's CI run timed out on `packages/core/src/gate.test.ts`'s performance AC test ("evaluates a batch of thousands of paths quickly...") — that test belongs to Story 3.2 part 1 (already merged), not this spec, but the fix landed on this branch since that's where the failing PR needed to go green. Root cause: the test's ~3,000-decoy-file setup phase (not `gate()`'s own measured work) exceeded vitest's 5-second default test timeout on a slower CI filesystem — Windows GitHub Actions runners are well known to be significantly slower for many-small-file I/O (commonly attributed to antivirus scanning overhead). The test's own internal performance assertion (`elapsedMs < 3000`, scoped only to the `gate()` call itself) was never at risk; only the outer, unrelated setup phase needed more wall-clock headroom. Fixed by adding an explicit 30-second timeout as the test's third argument — a scope, not a strictness, change: the real performance guarantee is untouched. Re-ran the full suite locally (245/245, macOS) and pushed; will confirm the `windows-latest` leg goes green on the next CI run.
