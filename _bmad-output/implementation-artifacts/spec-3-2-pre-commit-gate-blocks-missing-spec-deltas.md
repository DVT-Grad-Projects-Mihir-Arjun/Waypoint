...
- 2026-08-24: Post-merge-request fix. The cross-platform CI matrix (Story 1.4, `windows-latest` leg) failed two `gate.test.ts` config-error assertions: `CONFIG_RELATIVE_PATH` (`gate-classify.ts`) was defined via `path.join('.waypoint', 'config.yaml')`, which resolves to a backslash-separated path on Windows — `.waypoint\config.yaml` instead of the `.waypoint/config.yaml` every test and every other repo-relative path in this module expects. This was a regression I introduced myself during the prior patch round: reusing the shared constant (to fix the drift risk a reviewer flagged) inherited its pre-existing OS-native separator, which had never been exercised cross-platform before `gate()` started using it for a violation's `file` field. Fixed by making `CONFIG_RELATIVE_PATH` a forward-slash literal instead of a `path.join(...)` call — confirmed via `path.win32.join()` directly that a forward-slash literal segment still resolves correctly to a valid absolute Windows path for the actual file read, so this is safe on every platform. Re-ran the full suite locally (226/226, macOS) and pushed; confirm the `windows-latest` CI leg goes green on the next run.
