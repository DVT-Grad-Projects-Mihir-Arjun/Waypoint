...
| Git integration | Husky (or hand-rolled hook script) | Standard, well-supported pre-commit hook management. Lefthook (Go binary, no Node runtime dependency) is a viable lighter-weight alternative, particularly for non-Node consuming repos, but Husky is the MVP default since it needs no extra binary in a Node-toolchain repo |
...
Both compute a list of changed file paths themselves (pre-commit: `git diff --cached --name-only`; CI: `git diff <base>...<head> --name-only`) and pass that list, not a commit or ref, into the shared function:
...
5. **Pre-commit hook** (blocking, runs before the commit object exists): calls `gate()` with the staged file list — verifies Feature/System-tier changes have a corresponding spec delta (FR7). This is the *only* thing the pre-commit hook does; it has no involvement in task verification.
...
7. **CI** (`npx waypoint gate --ci`) does two independent, read-only checks over the PR — it writes nothing anywhere:
   ...
   - **Done-claim correctness**: for every task the ledger claims is `done` as of the PR's HEAD (enumerated by globbing `tasks/**/*.ledger.yaml` from the repo root), confirms `linked_commit` is a real commit and an ancestor of HEAD (see Components → Gate Script & Verification for why this is a cheap ancestor check, not a full re-run of `check_command`). A `done` task whose `linked_commit` doesn't resolve or isn't an ancestor fails the build with a clear message naming the task — this is the mechanical, agent-proof guarantee FR6 requires, enforced independently of whatever happened on any single contributor's machine, and cheap enough that its cost doesn't grow with the number of completed tasks.
...
8. `waypoint status` reads the local ledger directly — no remote-awareness needed, since nothing writes to the ledger except an explicit local `waypoint verify` (or a `git pull` of someone else's, or someone else's push). Spec is closed when all tasks are `done`.
...
- **Integration tests**: full CLI flows (`new-feature` → `approve` → `verify` → `gate --ci` pass) against a scratch git repo fixture.
