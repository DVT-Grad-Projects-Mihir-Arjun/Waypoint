...
- AC3: Default config generated by `waypoint install` pre-populates the following as patch-classified: `specs/patches/**`, `docs/**`, and root-level `*.md` files — so a fresh install doesn't over-enforce out of the box.
